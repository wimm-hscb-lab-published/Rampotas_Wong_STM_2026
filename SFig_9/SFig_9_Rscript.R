...
# Define colors: Grey for background, distinct color for "Alt" cells


highlight_colors <- c("Alt" = "red", "No Call" = "grey80", "Ref"="#3F778C")



DimPlot(MPN_object, reduction = "rpca_umap_3", group.by = "CALRins5_grouped", 
        pt.size = 1, cols = highlight_colors) +
  ggtitle("CALRins5 mutant cells") +
  theme_minimal() +
  guides(color = guide_legend(override.aes = list(alpha = 1))) +  # Ensure legend remains visible
  theme(panel.grid.major = element_blank(),  # Remove major grid lines
        ...
        axis.ticks = element_blank()) +  # Remove axis ticks  # Keep legend for reference
  scale_alpha_manual(values = c("Alt" = 1, "Other" = 0.2), guide = "none")  # Reduce transparency for background points

# 

# 9B: QC metrics for vartrix calls



vartrix_colors <- c("0" = "grey80", "Alt" = "#dc0000ff", "Ref" = "#3F778C")
# 
p1 <- VlnPlot(MPN_object, features = "nCount_RNA", group.by = "CALRins5_grouped", pt.size = 0) +
  ggtitle("Read Depth (nCount_RNA)") +
  theme_minimal() +
  scale_fill_manual(values = vartrix_colors) +
  theme(
    axis.line = element_line(size = 1),
    axis.ticks.y = element_line(size = 1),
    plot.title = element_text(hjust = 0.5, size = 14),  # Center and resize titles
    axis.text.x = element_text(angle = 45, hjust = 1)   # Rotate x-axis labels
  )
p1

p2 <- VlnPlot(MPN_object, features = "nFeature_RNA", group.by = "CALRins5_grouped", pt.size = 0) +
  ggtitle("Gene Count (nFeature_RNA)") +
  theme_minimal() +
  scale_fill_manual(values = vartrix_colors) +
  theme(
    axis.line = element_line(size = 1),
...
p3 <- VlnPlot(MPN_object, features = "mt.prop", group.by = "CALRins5_grouped", pt.size = 0) +
  ggtitle("Mitochondrial % (mt.prop)") +
  theme_minimal() +
  scale_fill_manual(values = vartrix_colors) +
  theme(
    axis.line = element_line(size = 1),
...
plot_row <- plot_grid(p1, p2, p3, ncol = 3)
plot_row
